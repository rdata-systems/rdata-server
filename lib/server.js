...
function RDataServer(options){
    ...
    self.initWebsocketServer = function(callback){
        ...
        // Usually callback is called when the websocket server starts.
        // If websocket server is provided, let's call the callback ourselves
        if(options.server)
            callback(null);
    };
