...
function RDataContextController(db, eventController){
    var self = this;
    self.db = db;
    self.eventController = eventController;

    self.init = function(callback){
        db.collection(contextCollectionName).createIndex(
            {"userId": 1},
            null,
            function(err, result) {
    ...
    self.startContext = function(connection, params, callback) {
        ...
        self.db.collection(contextCollectionName).insertOne(context, function(err, result) {
            if(err) {
                ...
                self.eventController.logEvent(connection, eventOptions, callback);
            }
        });
    };
