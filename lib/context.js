 ...
 * @param data
 * @param status
 * @param contextDataVersion
 * @param gameVersion
 * @constructor
 */
function RDataContext(user, id, name, parentContextId, parentContextPath, timeStarted, data, status, contextDataVersion, gameVersion){
    var self = this;
    ...
    self.children = [];
    self.data = data || null;
    self.contextDataVersion = contextDataVersion || 1;
    ...
    if(parentContextPath){
        self.path = parentContextPath.slice();
        self.path.push(parentContextId);
    }
}
...
function RDataContextController(server){
    ...
    self.startContext = function startContext(user, contextId, contextName, parentContextId, timeStarted, data, contextDataVersion, callback){
        if(parentContextId){ // If parent context id is provided, find it and add this one to it's children
            self.findAndRestoreContext(user, parentContextId, timeStarted, function onContextFound(error, parentContext){
                var context = new RDataContext(user, contextId, contextName, parentContextId, parentContext.path, timeStarted, data, contextStatusStarted, contextDataVersion, user.connection.gameVersion);
            ...
            var context = new RDataContext(user, contextId, contextName, null, null, timeStarted, data, contextStatusStarted, contextDataVersion, user.connection.gameVersion);

            self.findContext(user, contextId, function(error, existingContext){
                if(error) return callback(error);
                if(existingContext) return callback(new exceptions.ContextValidationError()); // Duplicate context
    ...
    self.startContextExposed = function(connection, params, callback) {
        ...
        self.startContext(connection.user, params.id, params.name, params.parentContextId, timeStarted, params.data, params.contextDataVersion, callback);
    };
