...
function RDataContextController(server){
    ...
    self.endContext = function endContext(user, contextId, timeEnded, callback){
        self.findAndRestoreContext(user, contextId, timeEnded, function onContextValidation(error, context){
            if(error) return callback(error);
    ...
    self.restoreInterruptedContexts = function restoreContexts(user, timeRestored, callback){
        self.db.collection(contextCollectionName).updateMany(
            ...
            function(err, result) {
                ...
                var eventOptions = {
                    name: "RestoreInterruptedContextsEvent",
                    data: {
                        timeRestored: timeRestored
                    }
                };
    ...
    self.endInterruptedContexts = function closeInterruptedContexts(user, timeEnded, callback){
        self.db.collection(contextCollectionName).updateMany(
            { userId: user.userId, status: contextStatusInterrupted },
            { "$set": { "status": contextStatusEnded, "timeEnded": timeEnded } },
            function(err, result) {
                ...
                var eventOptions = {
                    name: "EndInterruptedContextsEvent",
                    data: {
                        timeRestored: timeEnded
                    }
                };
                self.server.controllers.eventController.logEvent(user, eventOptions, callback);
            }
    ...
    self.restoreInterruptedContextsExposed = function(connection, params, callback){
        callback = typeof callback == "function" ? callback : function(){};
        var timeRestored = Date.now();

        self.restoreInterruptedContexts(connection.user, timeRestored, callback);
    };

    self.endInterruptedContextsExposed = function(connection, params, callback){
        callback = typeof callback == "function" ? callback : function(){};
        var timeEnded = Date.now();

        self.endInterruptedContexts(connection.user, timeEnded, callback);
    };
    ...
    self.setContextDataExposed = function setContextDataExposed(connection, params, callback){
        ...
        self.findAndRestoreContext(connection.user, params.id, Date.now(), function onContextFoundAndRestored(error, context) {
           if(error)
    ...
    self.exposed = {
        ...
        restoreInterruptedContexts: self.restoreInterruptedContextsExposed,
        endInterruptedContexts: self.endInterruptedContextsExposed,
